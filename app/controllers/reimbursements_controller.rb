...
class ReimbursementsController < ApplicationController
  ...
  def show
    if @current_user['manager'] == true
      @user = Reimbursement.select('user_id', 'id', 'title','description', 'approved')
      render json: @user.as_json, status: :ok
    else
      @user = Reimbursement.select('user_id', 'id', 'title','description', 'approved').where(user_id: @current_user['id'])
      if @user.empty?
